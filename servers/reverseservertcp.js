...
/**
 * Class making ModbusTCP reverse server.
 *
 ...
 * @constructor
 */
var ReverseServerTCP = function(vector, options) {
    var modbus = this;
    options = options || {};
    var recvBuffer = Buffer.from([]);

    // create a tcp client
    modbus._server = new net.Socket();
    ...
    modbus._server.on("connect", function(err) {
        modbusSerialDebug(JSON.stringify({ action: "connect" }));

        modbus.emit("connect", err);
    });
    ...
    modbus._server.on("close", function(err) {
        modbusSerialDebug(JSON.stringify({ action: "socket closed", data: err }));

        modbus.emit("closed", err);
    });
...
ReverseServerTCP.prototype.sendException = function(callback, functionCode, exceptionCode) {

    var outTcp = Buffer.alloc(9);
    outTcp.writeUInt16BE(0, 0); // transaction ID
    outTcp.writeUInt16BE(0, 2); // protocol ID (0 for Modbus/TCP)
    outTcp.writeUInt16BE(3, 4); // number of bytes remaining in frame
    outTcp.writeUInt8(this.unitID, 6); // UnitID
    outTcp.writeUInt8(parseInt(functionCode) | 0x80, 7); // function code, with MSB on byte set to 1 to signal exception
    outTcp.writeUInt8(parseInt(exceptionCode), 8); // exception code

    modbusSerialDebug({ action: "sedn exception", data: outTcp });
    modbusSerialDebug(JSON.stringify({ action: "sedn exception string", data: outTcp }));

    this._server.write(outTcp, null, callback);
};
...
* @param callback
*/
ReverseServerTCP.prototype.end = function() {
    this._server.removeAllListeners("data");
    this._server.end();

    modbusSerialDebug({ action: "end server" });
};
 ...
 * @param callback
 */
ReverseServerTCP.prototype.destroy = function(error) {
    if (!this._server.destroyed) {
        this._server.removeAllListeners("data");
        this._server.destroy(error);
    }
};
 ...
 * @type {ServerTCP}
 */
module.exports = ReverseServerTCP;
